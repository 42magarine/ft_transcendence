...
export default class Button extends AbstractView {
    ...
    async renderButton({ id, text, className = '', type = 'button', onClick = '', href, }) {
    ...
    async renderGroup({ buttons, align = 'left', layout = 'group', columns = 2, }) {
        const layoutClasses = {
            group: 'btn-group',
            stack: 'btn-stack',
        ...
        const renderedButtons = await Promise.all(buttons.map(btn => this.renderButton(btn)));
        return this.render(`
