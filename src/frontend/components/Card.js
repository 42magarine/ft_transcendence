...
import { themedCard, themedInput, themedBtn } from '../theme/themeHelpers.js';
export default class Card extends AbstractView {
    theme;
    constructor(params = new URLSearchParams()) {
        super(params);
        this.theme = (params.get('theme') || this.props.theme || 'default');
    }
        ...
        switch (block.type) {
            ...
            case 'toolbar':
                return `<div class="flex gap-2">${block.props.buttons
                    .map(btn => `<button onclick="${btn.onClick}" class="${themedBtn(this.theme)} btn-sm btn-secondary">${btn.text}</button>`)
                    .join('')}</div>`;
            default:
                return '';
        }
    }
    async renderCard({ title = '', footer = '', className = '', body = '', inputs = [], button, formId, extra = '', contentBlocks = [], theme = this.theme, }) {
        ...
        const formHtml = inputs.length || button
            ? `<form id="${formId || ''}">
    ...
    async renderGroup({ cards, layout = 'grid', className = '', theme = this.theme, }) {
        ...
        const renderedCards = await Promise.all(cards.map(config => this.renderCard({ ...config, theme })));
        return this.render(`
