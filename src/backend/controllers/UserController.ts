...
export class UserController {
    ...
    async register(request: FastifyRequest<{ Body: RegisterCredentials }>, reply: FastifyReply) {
        try {
            ...
            // If token exists, verify it and get the role -> should also redirect into user because registration is already verified...
            if (token) {
                try {
    ...
    async updateUser(request: FastifyRequest<{ Params: { id: string }, Body: Partial<UserModel> }>, reply: FastifyReply) {
        try {
            ...
            // If trying to change role, check permissions <--allow admins to change themselves to users?
